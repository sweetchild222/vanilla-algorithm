...
def train(X, T, convol_size, convol_stride, pool_size, pool_stride, learning_rate, iterate):
    ...
    for i in range(iterate):
        ...
        y = convolution_forward(X, c1_weight, c1_bias, convol_stride)
        ...
        error, weight_delta, bias_delta = convolution_backward(X, error, c1_weight, c1_bias, convol_stride)
...
c1_weight, c1_bias = train(train_x, train_t, convol_size, convol_stride, pool_size, pool_stride, learning_rate = 0.001, iterate = 50000)

y = convolution_forward(test_x, c1_weight, c1_bias, convol_stride)
s = activation_forward(y)
